...
Claude receives messages tagged with room ID, room name, sender, and event ID, and can respond using three tools:

- **reply** — send a plain text or HTML message to a Matrix room; optional `reply_to_event_id` routes into a thread under that event
- **react** — react to a specific message with an emoji
- **edit_message** — update a prior bot-authored message in-place via `m.replace`; useful for interim progress posts that shouldn't push-notify
...
### Per-call threading (`reply_to_event_id`)

The `reply` tool accepts an optional `reply_to_event_id` parameter. When set, the
message is posted as a threaded reply under the referenced event, with proper
`m.in_reply_to` fallback so unthreaded clients (FluffyChat) still see it. Read
the inbound `event_id` from the `<channel>` tag and pass it back for any
follow-up that should land inside the thread rather than top-level.

The existing `MATRIX_THREADS` static-per-project root mode still functions when
`reply_to_event_id` is omitted.

### `edit_message` tool

`edit_message(room_id, event_id, text, html?)` updates a prior bot-authored
message in-place via `m.replace`. Useful for interim progress posts that
shouldn't push-notify. The bot can only edit messages it sent itself; trying
to edit a user message returns `{ok: false, error: "not_owned_by_bot"}`. Always
follow up with a new `reply` to wake the user when the work completes — edits
don't generate push notifications.

### Typing indicator (`MATRIX_TYPING`)

The plugin fires a `PUT /rooms/:room/typing/:user` (timeout 30 s) on each
inbound allowed message, fire-and-forget. This shows a "…is typing" indicator
in client UIs while Claude processes the turn. **Default: on.** Set
`MATRIX_TYPING=false` in the environment to disable.

### `replyToMode` (access.json)

Set `replyToMode: "first" | "all" | "off"` in `access.json` to control whether
multi-chunk replies repeat the thread anchor on chunks 2+. Defaults to
`"first"` (matches Discord plugin default). `"off"` controls multi-chunk
continuation only — per-call `reply_to_event_id` always routes.
